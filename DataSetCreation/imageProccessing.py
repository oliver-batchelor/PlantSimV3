...
def pyrDownSmple(file_path):
    ...
    ann_noise = ann_blured + 20*np.random.rand(75, 75)
...
def AddRandomBackground(frgnd_img_path, bckgnd_img_dir):
    ...
    ret, frgnd_mask = cv2.threshold(cv2.cvtColor(frgnd_image, cv2.COLOR_BGR2GRAY),20,255, cv2.THRESH_BINARY_INV)
    ...
    # cv2.imshow('test', bckgnd_masked)
    # cv2.imshow('test1', bckgnd_masked)
    # cv2.imshow('test2', frgnd_bckgnd)
    # cv2.waitKey(10000)
